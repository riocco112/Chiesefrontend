...
import { Search, Star, ShoppingCart, Heart, Zap, ShieldCheck, Clock, Coins, Trophy, Sparkles, Menu, BadgeCheck, Flame } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          ...
          <div className="flex-1 max-w-xl mx-auto hidden md:block">
            <div className="relative">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari jasa joki, seller, paket coin…" className="w-full bg-pink-50 border border-pink-100 rounded-full pl-10 pr-4 py-2.5 text-sm placeholder:text-pink-300 text-slate-700 focus:outline-none focus:border-pink-300 focus:bg-white transition" />
            </div>
          </div>
          <button className="relative grid place-items-center w-10 h-10 rounded-full bg-pink-50 border border-pink-100 hover:bg-pink-100 transition">
            <ShoppingCart className="w-5 h-5 text-rose-400" />
            {cart > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 grid place-items-center text-[10px] font-bold rounded-full bg-rose-500 text-white">{cart}</span>}
          </button>
          <button className="hidden sm:block px-5 py-2 rounded-full text-sm font-semibold bg-gradient-to-r from-pink-400 to-rose-400 text-white shadow-md shadow-pink-300/50 hover:scale-[1.03] transition">Masuk</button>
          <button className="md:hidden grid place-items-center w-10 h-10 rounded-full bg-pink-50 border border-pink-100"><Menu className="w-5 h-5 text-rose-400" /></button>
        </div>
      </header>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14 sm:py-20">
          <div className="reveal max-w-2xl">
            ...
            <div className="flex flex-wrap gap-3">
              <button className="px-6 py-3 rounded-full font-semibold text-white bg-gradient-to-r from-pink-400 to-rose-400 shadow-lg shadow-pink-300/50 hover:scale-[1.02] transition">Jelajahi Jasa</button>
              <button className="px-6 py-3 rounded-full font-semibold text-rose-400 bg-white border border-pink-200 hover:bg-pink-50 transition">Jadi Seller</button>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 pb-20">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-slate-400"><Search className="w-10 h-10 mx-auto mb-3 opacity-40" />Nggak ada jasa yang cocok. Coba kata kunci lain.</div>
        ) : (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
            {filtered.map((l, i) => {
              const CatIcon = CATEGORIES.find((c) => c.id === l.category)?.icon ?? Sparkles;
              return (
                <article key={l.id} className="reveal group rounded-3xl overflow-hidden bg-white border border-pink-100 shadow-sm hover:shadow-xl hover:shadow-pink-200/40 hover:-translate-y-1 transition-all duration-300" style={{ animationDelay: `${i * 50}ms` }}>
                  <div className={`relative aspect-[4/3] overflow-hidden bg-gradient-to-br ${l.grad}`}>
                    ...
                    <button onClick={() => toggleFav(l.id)} className="absolute top-2.5 right-2.5 z-10 grid place-items-center w-8 h-8 rounded-full bg-white/95 backdrop-blur-md shadow-sm hover:scale-110 transition"><Heart className={`w-4 h-4 transition ${favs.has(l.id) ? 'text-rose-500' : 'text-pink-300'}`} fill={favs.has(l.id) ? 'currentColor' : 'none'} /></button>
                  ...
                  <div className="p-3 sm:p-4">
                    ...
                    <div className="flex items-end justify-between mt-3">
                      ...
                      <button onClick={() => setCart((c) => c + 1)} className="grid place-items-center w-9 h-9 rounded-full bg-gradient-to-r from-pink-400 to-rose-400 shadow-md shadow-pink-300/50 hover:scale-110 transition"><ShoppingCart className="w-4 h-4 text-white" /></button>
                    </div>
                  </div>
                </article>
              );
      ...
      <footer className="border-t border-pink-100 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 grid sm:grid-cols-4 gap-8 text-sm">
          <div className="sm:col-span-2"><div className="mb-3"><Logo /></div><p className="text-slate-400 max-w-xs leading-relaxed">Marketplace jasa joki Heartopia terpercaya. Transaksi aman dengan garansi & seller terverifikasi.</p></div>
          <div><h4 className="font-semibold mb-3 text-slate-600">Marketplace</h4><ul className="space-y-2 text-slate-400"><li className="hover:text-rose-400 cursor-pointer transition">Semua Jasa</li><li className="hover:text-rose-400 cursor-pointer transition">Jadi Seller</li><li className="hover:text-rose-400 cursor-pointer transition">Cara Order</li></ul></div>
          <div><h4 className="font-semibold mb-3 text-slate-600">Bantuan</h4><ul className="space-y-2 text-slate-400"><li className="hover:text-rose-400 cursor-pointer transition">FAQ</li><li className="hover:text-rose-400 cursor-pointer transition">Kontak</li><li className="hover:text-rose-400 cursor-pointer transition">Garansi</li></ul></div>
        </div>
        <div className="border-t border-pink-100 py-5 text-center text-xs text-slate-400">© 2026 Chiescaciy 甜心 · Bukan afiliasi resmi Heartopia / XD Entertainment</div>
      </footer>
    </div>
  );
}
