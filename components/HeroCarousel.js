...
const VIDEOS = [
  'https://res.cloudinary.com/duu9uutzz/video/upload/f_auto,q_auto:good/lv_0_20260603195106_ygtmyc.mp4',
  'https://res.cloudinary.com/duu9uutzz/video/upload/f_auto,q_auto:good/lv_0_20260617145149_g1aovy.mp4',
];
const DURATIONS = [10000, 33000, 33000]; // teks, video1, video2
...
export default function HeroCarousel() {
  ...
  const videoRefs = [null, video1Ref, video2Ref];

  // auto-slide timer sesuai durasi tiap slide
  useEffect(() => {
    const t = setTimeout(() => setIdx((i) => (i + 1) % 3), DURATIONS[idx]);
    return () => clearTimeout(t);
  }, [idx]);

  // play/pause video sesuai slide aktif
  useEffect(() => {
    [video1Ref, video2Ref].forEach((ref, i) => {
      const v = ref.current;
      if (!v) return;
      if (idx === i + 1) { v.currentTime = 0; v.play().catch(() => {}); }
      else { v.pause(); }
    });
  }, [idx]);

  const go = (n) => setIdx(((n % 3) + 3) % 3);
  ...
  const VideoSlide = ({ videoRef, src }) => (
    <div className="w-full shrink-0 pl-1 flex items-center">
      <div className="relative w-full">
        <video ref={videoRef} src={src} className="w-full rounded-2xl shadow-lg shadow-pink-200/50 bg-black" muted={muted} loop playsInline preload="metadata" style={{ aspectRatio: '16 / 9' }} />
        <button onClick={(e) => { e.stopPropagation(); setMuted(m => !m); }} aria-label={muted ? 'Nyalakan suara' : 'Matikan suara'} className="absolute bottom-3 right-3 grid place-items-center w-10 h-10 rounded-full bg-black/55 hover:bg-black/75 text-white transition backdrop-blur-sm">
    ...
    <div className="group relative max-w-2xl select-none" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      <div className="overflow-hidden rounded-3xl">
        <div className="flex transition-transform duration-500 ease-out" style={{ transform: `translateX(-${idx * 100}%)` }}>
          ...
          {/* SLIDE 2 - VIDEO 1 */}
          <VideoSlide videoRef={video1Ref} src={VIDEOS[0]} />

          {/* SLIDE 3 - VIDEO 2 */}
          <VideoSlide videoRef={video2Ref} src={VIDEOS[1]} />
